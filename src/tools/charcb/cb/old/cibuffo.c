...
void log_reset(CircularBuffer *cb) {
    fprintf(log_fifoInsert, "RESET\n");
}
...
size_t cb_getoffset(CircularBuffer *cb) {
    long distance = cb->fi_index - cb->cu_index;
    if (distance <= 0)distance += cb->elements;
    return (size_t) distance;
}

size_t cb_setoffset(size_t offset, CircularBuffer *cb) {
    long distance = cb->fi_index - cb->cu_index;
    if (distance <= 0)distance += cb->elements;
    return (size_t) distance;
}
...
#if DEBUG_FIFO_VERBOSE_LOG
        if (DEBUG_ENABLED) log_insertion(item, cb);
#endif
        return 0;
    }
...
void _updatenext(CircularBuffer *cb) {
    ...
    if (cb->cu_index == cb->fi_index) cb->hasNext = false;
}

size_t cb_next(void *item, CircularBuffer *cb) {

    if (cb_hasnext(cb) == false) {
...
#if DEBUG_FIFO_VERBOSE_LOG
    if (DEBUG_ENABLED)
        fprintf(log_fifoInsert, "[= setid    ] %ld"
                "\n", id);
#endif
